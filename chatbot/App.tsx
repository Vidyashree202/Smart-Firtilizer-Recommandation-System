...
const App: React.FC = () => {
  ...
  const handleSendMessage = async (userInput: string) => {
    ...
    // If offline, answer locally and return
    if (typeof navigator !== 'undefined' && navigator && navigator.onLine === false) {
      const offlineAnswer = getOfflineAnswer(userInput);
      if (offlineAnswer) {
        setMessages(prev => {
          const lastBot = [...prev].reverse().find(m => m.sender === MessageSender.BOT);
          if (lastBot && lastBot.text.trim() === offlineAnswer.trim()) {
            return prev; // skip duplicate
          }
          return [...prev, { id: Date.now() + 1, text: offlineAnswer, sender: MessageSender.BOT }];
        });
        setIsThinking(false);
        setSuggestionChips([
          { text: "ಸಹಾಯ", icon: "❓" },
        ...
        const fallback = [
          "ಆಫ್‌ಲೈನ್‌ ಮೋಡ್. ನಾನು ಕೆಲವು ಮೂಲಭೂತ ಪ್ರಶ್ನೆಗಳಿಗೆ ಮಾತ್ರ ಉತ್ತರಿಸಬಲ್ಲೆ.",
          ...
          "- ಹವಾಮಾನ / Weather",
          "- ನೆರವು / Irrigation",
          "- ಹುಳು/ರೋಗ / Pests & diseases",
          "- ಸುರಕ್ಷತೆ / Safety",
          "- ಸಂಗ್ರಹ / Storage",
        ].join("\n");
        setMessages(prev => {
          const lastBot = [...prev].reverse().find(m => m.sender === MessageSender.BOT);
          if (lastBot && lastBot.text.trim() === fallback.trim()) {
            return prev; // skip duplicate
          }
          return [...prev, { id: Date.now() + 1, text: fallback, sender: MessageSender.BOT }];
        });
        setIsThinking(false);
        setSuggestionChips([
          { text: "ಸಹಾಯ", icon: "❓" },
